...
// SalesQuotationTable component
const SalesQuotationTable =React.memo((props) => {
// add  this varibel  
...
const tableContainerRef = useRef(null);
  ...
  }, [props.Qut1Data.length]);


  const handleCalculations = (lineNum) => {
    props.setQut1Data(prevDln1 => {
  ...
  // Update line numbers for all entries in Qut1Data
  useEffect(() => {
    props.setQut1Data((prevIge12) =>
  ...
  const POR1D = () => {
    if (props.allFieldsFilled()) {
      ...
      props.setQut1Data([
        ...
        {
          DocEntry: "",
          LineNum: props.Qut1Data.length + 1, // Set the line number for the new entry
          ItemCode: "",
          ItemName: "",
          Quantity: 1,
          WhsCode: "",
  ...
  const handleEdit = (index, fieldName) => {
    ...
    if (props.Qut1Data[index].isEditing || props.state.Mode !== "Créer") {
       return;
    }

    if (props.allFieldsFilled()) {
       const updatedRows = props.Qut1Data.map((row, rowIndex) => ({
          ...row,
          isEditing: rowIndex === index,
       }));
       props.setQut1Data(updatedRows);
 ...
 useEffect(() => {
  ...
  return (
    <Box>
      ...
      {/* Show the "Add Row" button only in certain modes */}
      ...
      >
        <Table border="1" style={{ width: "100%", tableLayout: "fixed" }}>
          ...
          {/* Iterate over Qut1Data to display rows */}
          <TableBody>
            {props.Qut1Data.map((dln, index) => (
               //modifier
                <TableRow key={index}            
              onClick={(e) => {
              ...
              {/* modifier -------------------------------------------------------- */}
                ...
                {dln.isEditing ? (
                  ...
                  <TextField
                    ...
                    type="number"
                    name="Quantity"
                    // value={dln.Quantity}  // Changed from defaultValue to value
                    inputRef={(el) => (inputRefs.current[`Quantity-${index}`] = el)}
                    data-index={index}
                    onFocus={(e) => handleFocus(e.target.value)}
                    onBlur={(e) => handleBlur(dln.LineNum, e)}
                          ...
                          <FormControlLabel
                            control={
                              <Checkbox
                                ...
                                disabled={
                                  props.selectedField.name === "ItemCode" &&
                                  props.Qut1Data[props.selectedField.index] && // UPDATE
                                  props.Qut1Data[props.selectedField.index]
                                    .ItemCode &&
