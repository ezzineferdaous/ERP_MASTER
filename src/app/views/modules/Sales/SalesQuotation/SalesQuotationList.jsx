...
// Main component for the SalesQuotationList
const SalesQuotationList = (props) => {
  ...
  const [data, setData] = useState([]); // State to store OQUT data
  ...
  const Back = async (e) => {
    ...
    try {
      props.setShowOQUTList(false); // Hide the delivery list and go back
    } catch (error) {
  ...
  const handleClick = async (event, row) => {
    try {
      ...
      // Fetch QUT1 data (related to delivery lines) for the selected document
      const QUT1Response = await axiosInstance.get(
        `QUT1/data/List/${row.original.DocEntry}` // Fetch lines based on DocEntry
      );
      const Qut1Data = QUT1Response.data;
      ...
      // Update QUT1 data with names from Warehouse and UM
      const updatedQut1Data = Qut1Data.map((item) => ({
        ...item,
      ...
      props.setQut1Data(updatedQut1Data); // Update state with the fetched QUT1 date
    } catch (error) {
      console.error(error); // Log any error that occurs
    }
    props.setShowOQUTList(false); // Hide the delivery list
  };

  // Fetch OQUT (order) data when the component is mounted
  useEffect(() => {
    const getOQUT = async () => {
      setIsLoading(true); // Set loading to true when data is being fetched
      try {
        const res = await axiosInstance.get("OQUT"); // Fetch OQUT data from the API
      ...
      setRowCount(modifiedData.length);
        ...
        const respdf = await axiosInstance.get("OQUT/PDF");
    ...
    getOQUT(); // Call the function to fetch OQUT data
  }, []);
  ...
  const columns = useMemo(
    () => [
      ...
      { accessorKey: "TotalHT", header: "TotalHT", size: 130 }, // Column for TotalHT
  ...
  const table = useMaterialReactTable({
    ...
    renderTopToolbarCustomActions: ({ table }) => (
      ...
      >
        ...
        <XLSXDownload
          fileName="List Devis client" // Filename for Excel download
        ...
        />
        <PDFDownload
          fileName="List Devis client" // Filename for PDF download
          tableHeaders={exportColumns} // Columns to export
          disabled={dataPdf.length === 0} // Disable if no data
          data={dataPdf} // Data for export
