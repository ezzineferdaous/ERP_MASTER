...
const DeliveryTable = React.memo((props) => {
  // add  thes varibel
  ...
  // Update line numbers for all entries in Dln1Data
  useEffect(() => {
    props.setDln1Data((prevIge12) =>
  ...
  const POR1D = () => {
    if (props.allFieldsFilled()) {
      ...
      props.setDln1Data([
        ...
        {
          DocEntry: "",
          LineNum: props.Dln1Data.length + 1, // Set the line number for the new entry
          ItemCode: "",
          ItemName: "",
          Quantity: 1,
          WhsCode: "",
  ...
  const handleDelete = (index) => {
    ...
    const updatedDLN1 = [...props.Dln1Data]; // Clone the data
    updatedDLN1.splice(index, 1); // Remove the selected row
    props.setDln1Data(updatedDLN1); // Update the state
  ...
  const handleEdit = (index, fieldName) => {
    ...
    if (props.Dln1Data[index].isEditing || props.state.Mode !== "Créer") {
      return;
    }

    if (props.allFieldsFilled()) {
      const updatedRows = props.Dln1Data.map((row, rowIndex) => ({
        ...row,
        isEditing: rowIndex === index,
      }));
      props.setDln1Data(updatedRows);
                          ...
                          <FormControlLabel
                            control={
                              <Checkbox
                                ...
                                disabled={
                                  props.selectedField.name === "ItemCode" &&
                                  props.Dln1Data[props.selectedField.index] && // UPDATE
                                  props.Dln1Data[props.selectedField.index]
                                    .ItemCode &&
