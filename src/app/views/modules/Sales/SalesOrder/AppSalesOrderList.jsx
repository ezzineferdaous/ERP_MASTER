...
const handleClick = async (event, row) => {
  try {
    ...
    props.setoRDR(row.original);
    props.setEditIndex(null);

    // Fetch RDR1 data based on the selected DocEntry
    const RDR1Response = await axiosInstance.get(`RDR1/data/List/${row.original.DocEntry}`);
    const RDR1 = RDR1Response.data;
    ...
    // Update RDR1 data with names
    const updatedRPD1 = RDR1.map(item => ({
      ...item,
      WhsCode: warehouseMap[item.WhsCode] || item.WhsCode,
      UM: umMap[item.UM] || item.UM,
    }));

    props.setRDR1(updatedRPD1);
  } catch (error) {
    console.error(error);
  }
  props.setShowORDRList(false);
  ...
  useEffect(() => {
    const getORDR = async () => {
      setIsLoading(true);
      try {
        const res = await axiosInstance.get("ORDR");
        setData(res.data);
        ...
        const respdf = await axiosInstance.get("ORDR/PDF"); 
