...
import {Table, TableBody, TableCell, TableContainer,FormControl ,TablePagination, TableHead, TableRow, Select , Paper,  IconButton, TextField, Modal, Box,Radio, Typography, Button,  InputAdornment ,  FormControlLabel, Checkbox} from "@mui/material";
import { FontAwesomeIcon } from "@fortawesome/react-fontawesome";
import {  faBars,  faTrash,  faTimes,  faSearch,} from "@fortawesome/free-solid-svg-icons";
...
const CreditMemoTable =React.memo((props) => {
...
//edit this function
  const calculateValues = (lineNum) => {
    props.setRDR1(prevRdp1 => {
 ...
 const handleInputChange = useCallback((lineNum, event) => {
  ...
  console.log("handleInputChange :", props.RDR1);
}, [props.setRDR1]);
  ...
  // Add a new row to RPN if all fields are filled
  const RP1 = () => {
...
const handleDelete = (index) => {
  ...
  const updatedRIN1 = [...props.RDR1];
  updatedRIN1.splice(index, 1); // Remove the selected row
  props.setRDR1(updatedRIN1); // Update the state with the new list
  console.log("Data after delete:", updatedRIN1);
};
...
const handleEdit = (index, fieldName) => {
  ...
  if (props.RDR1[index].isEditing || props.state.Mode !== "Créer") {
    return;
  }

  if (props.allFieldsFilled()) {
    const updatedRows = props.RDR1.map((row, rowIndex) => ({
      ...row,
      isEditing: rowIndex === index
    }));
    props.setRDR1(updatedRows);
    ...
    <Box>
     <style>{styles}</style>
       {props.state.Mode !== "Mettre à jour" && props.state.Mode !== "OK" && (
        <Button onClick={RP1} variant="contained">
          <AddIcon fontSize="small" />
        </Button>
      )}
    <TableContainer   ref={props.state.Mode === "Créer" ? tableContainerRef : null}
     component={Paper} style={{ marginTop: "2%", maxHeight: props.state.Mode === "Créer" ? '265px' : '250px', overflowY: 'auto' }}>
      <Table border="1" style={{ width: "100%", tableLayout: "fixed" }}>
        <TableHead  style={{ backgroundColor: "lightgrey", position: 'sticky', top: 0,
         zIndex: 1 ,height: "20px"}}>
          <TableRow>
          <TableCellK style={{ width: "20px" }}>ID</TableCellK>
               <TableCellK style={{ display: "none" }}>Numéro Order</TableCellK>
               <TableCellK style={{ display: "none" }}>Numéro de ligne</TableCellK>
   ...
   // edet this [
    ...
    >
      {props.vatOptions.map((option, key) => {
        return (
          <MenuItem key={key} value={option.Rate}>
          ...
          <FormControlLabel
            control={
              <Checkbox
                ...
                disabled={
                  props.selectedField.name === "ItemCode" &&
                  props.RDR1[props.selectedField.index] &&
                  props.RDR1[props.selectedField.index].ItemCode &&
