...
const CreditMemoList = (props) => {
  ...
  //     const RPC1Response = await axiosInstance.get(`RIN1/data/List/${row.original.DocEntry}`);
...
const handleClick = async (event, row) => {
  try {
    props.setState({ ...props.state, Mode: "Mettre à jour" });
    props.setoRIN(row.original);
    props.setEditIndex(null);

    // Fetch RIN1 data based on the selected DocEntry
    const RPC1Response = await axiosInstance.get(`RIN1/data/List/${row.original.DocEntry}`);
    const RIN1 = RPC1Response.data;
    ...
    // Update RIN1 data with names
    const updatedRPD1 = RIN1.map(item => ({
      ...item,
      WhsCode: warehouseMap[item.WhsCode] || item.WhsCode,
      UM: umMap[item.UM] || item.UM,
    }));

    props.setRIN1(updatedRPD1);
  } catch (error) {
    console.error(error);
  }
  props.setShowORINList(false);
  ...
  useEffect(() => {
    const getORIN = async () => {
      setIsLoading(true);
      try {
        const res = await axiosInstance.get("ORIN");
        setData(res.data);
        ...
        const respdf = await axiosInstance.get("ORIN/PDF"); 
