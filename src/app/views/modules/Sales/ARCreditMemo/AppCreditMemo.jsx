...
import CreditMemoToolbar from "./CreditMemoToolbar";
...
const AppCreditMemo = () => {
  ...
  const [ShowORPDList, setShowORINList] = useState(false);
  ...
  // oRIN and RIN1 data state management
  ...
  const fetchNextDocEntry = async (nextMode) => {
    try {
      const response = await axiosInstance.get("/ORIN/data/MaxDoc");
  ...
  // Function to handle adding a new oRIN and RIN1
  const Add = async (e) => {
...
const formattedValue = typeof value === 'string' ? value.replace(',', '.') : value;
  ...
  const handleCalculate = () => {
    const totalHT = RIN1.reduce((acc, curr) => acc + parseFloat(curr.LineHT || 0), 0);
    const DiscPrcnt = (oRIN.DiscPrcnt) || 0;
    const RemiseTotal = isManualEntry
      ? (oRIN.RemiseTotal) || 0
      : (totalHT * DiscPrcnt) / 100;
    const totalARH = totalHT - RemiseTotal;
  
    const SmTva = RIN1.reduce((acc, curr) => {
  ...
  const handleSelectItem = (item) => {
    ...
    if (selectedField.name === "ItemCode" && RIN1[selectedField.index].ItemCode) {
      setSelectedItem([item]);
    } else {
      if (isSelected) {
  ...
  const handleSelect = (item) => {
    const updatediqr1 = RIN1.map((iqr, i) =>
  ...
  const handleSaveSelectedItem = () => {
    if (selectedField.name === "ItemCode") {
      ...
      const updatedRdp1 = RIN1.filter(row => row.ItemCode !== "");
 
      if (selectedItem.length > 0) {
        const currentRow = RIN1[selectedField.index];
  ...
  const prepareDataForSubmission = () => {
    return RIN1.map((RIN1) => {
      const umItem =
        modalContentItemCode.find((item) => item.NomUM === RIN1.UM)?.CodeUMA ||
        modalContentUM.find((item) => item.description === RIN1.UM)?.id;
      const whsCodeItem =
        modalContentItemCode.find((item) => item.NomWarehouse === RIN1.WhsCode)
          ?.Magasin ||
        modalContentWhsCode.find((item) => item.description === RIN1.WhsCode)
          ?.id;

      return {
        ...RIN1,
        UM: umItem,
        WhsCode: whsCodeItem || RIN1.WhsCode,
      };
    });
  };
  ...
  const allFieldsFilled = () => {
    return RIN1.every((por) => {
      return por.ItemCode;
    });
  };

  const allFieldsFilleddd = () => {
    if (!oRIN.CardCode) return `Saisir le code du Client`;
    if (!oRIN.CardName) return `Saisir le nom du Client`;

    for (const [index, por] of RIN1.entries()) {
      if (!por.ItemCode)
  ...
  const handleSubmit = async (event) => {
    ...
    if (state.Mode === "Créer") {
      try {
        // Save oRIN and RIN1 data
        await Promise.all([
          axiosInstance.post("oRIN", oRIN),
          ...preparedIqr1.map((item) =>
            axiosInstance.post("RIN1", item)
          ),
          ...preparedIqr1.map((item) =>
            axiosInstance.post(`Stock/${item.id ?? ""}`, {
              TransType: 14,
              DocNum: oRIN.DocNum,
              CardCode: oRIN.CardCode,
      ...
      try {
        // Update oRIN and RIN1 data
        await Promise.all([
          axiosInstance.put(`oRIN/${oRIN.id ?? ""}`, oRIN),
          ...preparedIqr1.map((item) =>
 ...
 console.log(oRIN);
  ...
  const end = (
    <CreditMemoToolbar
      Add={Add}
      setShowORINList={setShowORINList}
      RIN1={RIN1}
