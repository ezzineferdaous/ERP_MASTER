...
const ReturnsTable = React.memo((props) => {
// add  thes varibel  
...
//edit this function
  ...
  props.setRdn1(prevRdn1 => {
    
    const index = prevRdn1.findIndex(row => row.LineNum === lineNum);
    
    if (index === -1) return prevRdn1;

    const updatedRow = { ...prevRdn1[index] };
    ...
    console.log("handleCalculations - updated row:", prevRdn1);
    prevRdn1[index] = updatedRow;
...
//edit this function
  const handleInputChange = useCallback((lineNum, event) => {
    ...
    console.log("handleInputChange :", props.Rdn1);
  }, [props.setRdn1]);
// add  thes function 
...
// add  thes function 
  const handleEdit = (index, fieldName) => {
    ...
    if (props.Rdn1[index].isEditing || props.state.Mode !== "Créer") {
       return;
    }

    if (props.allFieldsFilled()) {
       const updatedRows = props.Rdn1.map((row, rowIndex) => ({
          ...row,
          isEditing: rowIndex === index,
       }));
       props.setRdn1(updatedRows);
      ...
      <Table border="1" style={{ width: "100%", tableLayout: "fixed" }}>
        <TableHead  style={{ backgroundColor: "lightgrey", position: 'sticky', top: 0, zIndex: 1 ,height: "20px"}}>
          <TableRow>
            ...
            <TableCellKVAT>TVA %</TableCellKVAT>
                    ...
                    <TableRow key={index} onClick={(e) => { if (e.target.type !== 'checkbox') { props.handleSelectItem(item);}}} style={{ cursor: 'pointer' }}>
                      ...
                      {props.selectedField.name === "ItemCode" ? (
                        ...
                        <Checkbox
                          ...
                          props.Rdn1[props.selectedField.index] && 
                          props.Rdn1[props.selectedField.index].ItemCode && 
