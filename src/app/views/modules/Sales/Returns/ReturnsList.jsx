...
const ReturnsList = (props) => {
  ...
  const handleClick = async (event, row) => {
    try {
      ...
      props.setOrdn(row.original);
      // Fetch RDN1 data based on the selected DocEntry
      const RDN1Response = await axiosInstance.get(`RDN1/data/Min/${row.original.DocEntry}`);
      const rdn1Data = RDN1Response.data;
      ...
      // Update RDN1 data with names
      const updatedRdn1Data = rdn1Data.map(item => ({
        ...item, WhsCode: warehouseMap[item.WhsCode] || item.WhsCode,
        UM: umMap[item.UM] || item.UM,
      }));
      props.setRdn1(updatedRdn1Data);
    } catch (error) {
      console.error(error);
    }
    props.setShowORDNList(false);
  ...
  useEffect(() => {
    const getOPDN = async () => {
      setIsLoading(true);
      try {
        const res = await axiosInstance.get("ORDN");
        setData(res.data);
      ...
      setRowCount(modifiedData.length);
        ...
        const respdf = await axiosInstance.get("ORDN/PDF");
  ...
  const table = useMaterialReactTable({
    ...
    renderTopToolbarCustomActions: ({ table }) => (
      <Box sx={{ display: 'flex', gap: '16px', padding: '8px', flexWrap: 'wrap' }}>
        <XLSXDownload fileName="Liste des retours client " tableHeaders={exportColumns} disabled={dataPdf.length === 0} data={dataPdf} Text={'Excel'} />
        <PDFDownload fileName="Liste des retours client " tableHeaders={exportColumns} disabled={dataPdf.length === 0} data={dataPdf} Text={'PDF'} />
      </Box>
    ),
