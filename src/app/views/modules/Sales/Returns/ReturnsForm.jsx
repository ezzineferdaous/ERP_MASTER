// frontend/src/components/ReturnsForm.js
...
const ReturnsForm = (props) => {
    const handleSearchIconClick = () => {props.setSearchInputVisible(!props.searchInputVisible);props.setSearchQuery("");};
    const handleChangePage = (event, newPage) => { props.setPage(newPage);};
    const handleChangeRowsPerPage = (event) => {props.setRowsPerPage(parseInt(event.target.value, 2)); props.setPage(0);};
    const handleClose = () => {props.setState({ ...props.state, open: false });};
    const { vertical, horizontal, open, message, severity } = props.state;
    useEffect(() => {  props.fetchNextDocEntry(); Modal.setAppElement('#root');}, []);
    ...
    useEffect(() => {
            if (!props.Ordn.DocStatus.trim()) {
                props.setOrdn(prevFormData => ({
                ...prevFormData,
                DocStatus: 'O',
            }));
            }
    }, [props.Ordn.DocStatus, props.setOrdn]);
    let content = [];
                ...
                <Grid container spacing={10} style={{marginTop:"-100px"}}>
                        ...
                        <FormControl fullWidth style={{ marginRight: "40px", width: "100%", marginBottom:"10px" }}>
                            ...
                            <Select size="small" 
                                ...
                                labelId="DocStatus"
                                name="DocStatus"
                                value={props.Ordn.DocStatus.trim() || "O"}
                                onChange={props.handleChange}
                                style={{ width: '100%' }}
                                inputProps={{ readOnly: true }}
                            >
                        ...
                        <div style={{ position: "relative", width: "100%" }}>
                            <Flatpickr
                              className="Flatpickr-flatpickr"
                                ...
                                value={props.Ordn.DocDate ? new Date(props.Ordn.DocDate) : new Date()}
                            ...
                            />
                            <label className="label-flatpickr" style={{top: props.Ordn.DocDate ? "-10px" : "12px"}} > Date Document </label>
                        </div>
                    </Grid>
                    <Grid item lg={4} md={6} sm={12} xs={12}>
                                    <div style={{ position: "relative", width: "100%" }}>
                                        ...
                                        className="Flatpickr-flatpickr"
                                            data-enable-time={false}
                                            value={props.Ordn.DueDate ? new Date(props.Ordn.DueDate) : new Date()}
                                            onChange={(date) => props.handleChange({  target: { name: "DueDate", value: date[0] }, }) }
                                            options={{ dateFormat: "d/m/Y"}}
                                            disabled={props.state.Mode === "Mettre à jour" || props.state.Mode === "OK"}
                                        />
                                        <label className="label-flatpickr" style={{top: props.Ordn.DueDate ? "-10px" : "12px"}} > Date d'écheance </label>
                                    </div>
                    </Grid>
                </Grid>
                <ReturnsTable handleCalculations2={props.handleCalculations2}
                 handleSearchIconClick={handleSearchIconClick} filterData={filterData} 
                 ...
                 setRdn1={props.setRdn1} setOpen={props.setOpen}setSelectedField={props.setSelectedField}
                 modalContent={ paginatedData }    Rdn1={ props.Rdn1}
                 ...
                 selectedField={props.selectedField}setOrdn={props.setOrdn}searchQuery={props.searchQuery}
