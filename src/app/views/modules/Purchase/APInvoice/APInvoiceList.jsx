...
const APInvoiceList = ({
  ...
  const handleClick = async (event, row) => {
    try {
      ...
      // Fetch Pch1 data based on the selected DocEntry
      const pch1Response = await axiosInstance.get(
        `PCH1/data/List/${row.original.DocEntry}`
      );
      const Pch1Data = pch1Response.data;
      ...
      // Update Pch1 data with names
      const updatedPch1Data = Pch1Data.map((item) => ({
        ...item,
        WhsCode: warehouseMap[item.WhsCode] || item.WhsCode,
        UM: umMap[item.UM] || item.UM,
      }));

      setAPIn(updatedPch1Data);
    } catch (error) {
      console.error(error);
    }
    setShowInvoiceList(false);
  ...
  useEffect(() => {
    const getInvoices = async () => {
      setIsLoading(true);
      try {
        const res = await axiosInstance.get("OPCH");
        setData(res.data);
        ...
        const respdf = await axiosInstance.get("OPCH/PDF"); 
  ...
  const table = useMaterialReactTable({
    ...
    renderTopToolbarCustomActions: ({ table }) => (
      ...
      >
        <XLSXDownload
          fileName="List Facture De Fournisseur"
