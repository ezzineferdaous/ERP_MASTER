...
const ARGoodsReturnTable = React.memo((props) => {
// add  thes varibel  
...
//edit this function
  ...
  props.setRdp1(prevRdp1 => {
    
    const index = prevRdp1.findIndex(row => row.LineNum === lineNum);
    
    if (index === -1) return prevRdp1;

    const updatedRow = { ...prevRdp1[index] };
    ...
    console.log("handleCalculations - updated row:", prevRdp1);
    prevRdp1[index] = updatedRow;
...
//edit this function
  const handleInputChange = useCallback((lineNum, event) => {
    ...
    console.log("handleInputChange :", props.Rdp1);
  }, [props.setRdp1]);
// add  thes function 
...
// add  thes function 
  const handleEdit = (index, fieldName) => {
    ...
    if (props.Rdp1[index].isEditing || props.state.Mode !== "Créer") {
       return;
    }

    if (props.allFieldsFilled()) {
       const updatedRows = props.Rdp1.map((row, rowIndex) => ({
          ...row,
          isEditing: rowIndex === index,
       }));
       props.setRdp1(updatedRows);
 ...
 useEffect(() => {
  ...
  return (
    ...
    <TableContainer ref={props.state.Mode === "Créer" ? tableContainerRef : null} component={Paper} style={{ marginTop: '2%', maxHeight: props.state.Mode === "Créer" ? '265px' : '250px', overflowY: 'auto' }}>
        ...
        <TableBody>
          {props.Rdp1.map((pdn, index) => (
            ...
            onClick={(e) => {
              ...
              <StyledTableCell style={{ textAlign: "center", marginLeft: "25px" }}> {index + 1} </StyledTableCell>
              {pdn.isEditing  ? (
                <>
                  <StyledTableCell>
                    <div style={{ position: "relative", display: "flex", alignItems: "center", }}>
                      ...
                      />
                      <FontAwesomeIcon icon={faBars} onClick={() => props.handleOpenModal("ItemCode", index)} style={{  position: "absolute", right: "10px", cursor: "pointer",}}/>
                    </div>
                  </StyledTableCell>
                  <StyledTableCell>
                    <TextField
                  ...
                  <StyledTableCell>
                    <div style={{ position: "relative", display: "flex",  alignItems: "center", }} >
                      <TextField
                        ...
                        InputProps={{style: { borderRadius: "0px", paddingRight: "15px" },}}
                      />
                      <FontAwesomeIcon icon={faBars} onClick={() => props.handleOpenModal("WhsCode", index)} style={{ position: "absolute", right: "10px", cursor: "pointer",}}/>
                    </div>
                  </StyledTableCell>
                  <StyledTableCell>
                    <TextField
                  ...
                  <StyledTableCell>
                    <FormControl fullWidth>
                      ...
                      >
                        {props.vatOptions.map((option , key) => {return <MenuItem key={key} value={option.Rate}>{`${option.Code} - ${option.Name}%`}</MenuItem>;})}
                      </Select>
                    </FormControl>
                  </StyledTableCell>
                  <StyledTableCell>
                  ...
                  <StyledTableCell>
                    <TextField
                      ...
                      InputProps={{ style: { borderRadius: "0px", appearance: "textfield" }, readOnly: true }}
                    />
                  </StyledTableCell>
                  <StyledTableCell>
                    <TextField
                  ...
                  <StyledTableCell>
                    <div style={{position: "relative",display: "flex",alignItems: "center",}}>
                      <TextField
                        ...
                        InputProps={{style: { borderRadius: "0px", paddingRight: "15px" },}}
                      />
                      <FontAwesomeIcon icon={faBars} onClick={() => props.handleOpenModal("UM", index)} style={{position: "absolute",right: "10px",cursor: "pointer",}}/>
                    </div>
                  </StyledTableCell>
                  {props.state.Mode !== "Mettre à jour" && props.state.Mode !== "OK" &&(
                  <StyledTableCell data-field="Action">
                    <IconButton sx={{ fontSize: 10,'&:hover': { backgroundColor: 'transparent' }}} style={{ marginLeft: '10px' }} onClick={(e) => {e.stopPropagation(); handleDelete(index);}}>
                ...
                <> 
                  ...
                  <StyledTableCell data-field="VAT" style={{ textAlign: "center" }}>{ props.vatOptions.find(option => option.Rate === pdn.VAT)?.Name || pdn.VAT}</StyledTableCell>
      ...
      <Modal  open={props.open} onClose={props.handleCloseModal} aria-labelledby="modal-modal-title" aria-describedby="modal-modal-description">
        <Box sx={modalStyle}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <Typography variant="h6" component="h2" > Liste : 
              {props.selectedField.name === "ItemCode" ? "Articles" : props.selectedField.name === "WhsCode" ? "Magasin": props.selectedField.name === "CardCode" ? "fournisseur" : "Unite de Mesure" }
            </Typography>
          ...
          </IconButton> {props.searchInputVisible &&
            <TextField
              placeholder="Search..."
              variant="outlined"
              size="small"
                    ...
                    <TableRow key={index} onClick={(e) => { if (e.target.type !== 'checkbox') { props.handleSelectItem(item);}}} style={{ cursor: 'pointer' }}>
                      ...
                      {props.selectedField.name === "ItemCode" ? (
                        ...
                        <Checkbox
                          ...
                          props.Rdp1[props.selectedField.index] && 
                          props.Rdp1[props.selectedField.index].ItemCode && 
...
export default  ARGoodsReturnTable;