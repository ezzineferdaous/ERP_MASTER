...
const APCreditMemoList = (props) => {
  ...
  //     // Fetch POR1 data based on the selected DocEntry
  //     const RPC1Response = await axiosInstance.get(`RPC1/data/List/${row.original.DocEntry}`);
  //     props.setRPD1(RPC1Response.data);
  //   } catch (error) {
  //     console.error(error);
  //   }
  //   props.setShowORPCList(false);
...
const handleClick = async (event, row) => {
  try {
    props.setState({ ...props.state, Mode: "Mettre à jour" });
    props.setoRPC(row.original);
    props.setEditIndex(null);

    // Fetch RPC1 data based on the selected DocEntry
    const RPC1Response = await axiosInstance.get(`RPC1/data/List/${row.original.DocEntry}`);
    const RPC1 = RPC1Response.data;
    ...
    // Update RPC1 data with names
    const updatedRPD1 = RPC1.map(item => ({
      ...item,
      WhsCode: warehouseMap[item.WhsCode] || item.WhsCode,
      UM: umMap[item.UM] || item.UM,
    }));

    props.setRPC1(updatedRPD1);
  } catch (error) {
    console.error(error);
  }
  props.setShowORPCList(false);
  ...
  useEffect(() => {
    const getORPC = async () => {
      setIsLoading(true);
      try {
        const res = await axiosInstance.get("ORPC");
        setData(res.data);
        ...
        const respdf = await axiosInstance.get("ORPC/PDF"); 
  ...
  const columns = useMemo(() => [
    ...
    { accessorKey: "DocNum", header: "N°", size: 60 },
    {
      accessorKey: 'DocDate',
    ...
    { accessorKey: "CardCode", header: "Code", size: 130 },
    { accessorKey: "CardName", header: "Nom", size: 130 },
    { accessorKey: "TotalHT", header: "Total HT", size: 130 },
    { accessorKey: "DiscPrcnt", header: "Remise%", size: 130 },
    { accessorKey: "RemiseTotal", header: " Total Remise", size: 130 },
    ...
    { accessorKey: "DocStatus", header: "Statut", size: 130 },
    {
  ...
  const table = useMaterialReactTable({
    ...
    renderTopToolbarCustomActions: ({ table }) => (
      <Box sx={{ display: 'flex', gap: '16px', padding: '8px', flexWrap: 'wrap' }}>
        <XLSXDownload fileName="List Réception De Marchandises " tableHeaders={exportColumns} disabled={dataPdf.length === 0} data={dataPdf} Text={'Excel'} />
        <PDFDownload fileName="List Réception De Marchandises " tableHeaders={exportColumns} disabled={dataPdf.length === 0} data={dataPdf} Text={'PDF'} />
      </Box>
    ),
...
export default APCreditMemoList;