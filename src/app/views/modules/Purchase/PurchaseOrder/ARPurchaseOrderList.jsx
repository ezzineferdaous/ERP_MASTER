...
// Main component for the ARPurchaseOrderList
const ARPurchaseOrderList = (props) => {
  ...
  const [data, setData] = useState([]); // State to store OPOR data
  ...
  const Back = async (e) => {
    ...
    try {
      props.setShowOPORList(false); // Hide the delivery list and go back
    } catch (error) {
  ...
  const handleClick = async (event, row) => {
    try {
      ...
      props.setOporData(row.original); // Set the selected document data

      // Fetch POR1 data (related to delivery lines) for the selected document
      const POR1Response = await axiosInstance.get(
        `POR1/data/List/${row.original.DocEntry}` // Fetch lines based on DocEntry
      );
      const POR1Data = POR1Response.data;
      ...
      // Update POR1 data with names from Warehouse and UM
      const updatedPOR1Data = POR1Data.map((item) => ({
        ...item,
      ...
      props.setPor1Data(updatedPOR1Data); // Update state with the fetched DLN1 dat
    } catch (error) {
      console.error(error); // Log any error that occurs
    }
    props.setShowOPORList(false); // Hide the delivery list
  };
  // Fetch OPOR (order) data when the component is mounted
  useEffect(() => {
    const getOPOR = async () => {
      setIsLoading(true); // Set loading to true when data is being fetched
      try {
        const res = await axiosInstance.get("OPOR"); // Fetch OPOR data from the API
        ...
        const respdf = await axiosInstance.get("OPOR/PDF");
    ...
    getOPOR(); // Call the function to fetch OPOR data
  }, []);
  ...
  const columns = useMemo(
    () => [
      ...
      { accessorKey: "RemiseTotal", header: " Total Remise", size: 130 }, // Column for RemiseTotal
      { accessorKey: "VatSum", header: "Montant TVA", size: 130 }, // Column for VatSum
      { accessorKey: "DocTotal", header: "DocTotal", size: 130 }, // Column for DocTotal
  ...
  const table = useMaterialReactTable({
    ...
    renderTopToolbarCustomActions: ({ table }) => (
      ...
      >
        ...
        <XLSXDownload
          fileName="List Commande De Fournisseur " // Filename for Excel download
        ...
        />
        <PDFDownload
          fileName="List Commande De Fournisseur " // Filename for PDF download
          tableHeaders={exportColumns} // Columns to export
          disabled={dataPdf.length === 0} // Disable if no data
          data={dataPdf} // Data for export
...
export default ARPurchaseOrderList;
