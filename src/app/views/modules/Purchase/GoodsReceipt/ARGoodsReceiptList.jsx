...
const ARPurchaseOrderList = (props) => {
  ...
  const handleClick = async (event, row) => {
    try {
      ...
      props.setOpdnData(row.original);

      // Fetch PDN1 data based on the selected DocEntry
      const PDN1Response = await axiosInstance.get(
        `PDN1/data/List/${row.original.DocEntry}`
      );
      const pdn1Data = PDN1Response.data;
      ...
      // Update PDN1 data with names
      const updatedPdn1Data = pdn1Data.map((item) => ({
        ...item,
        WhsCode: warehouseMap[item.WhsCode] || item.WhsCode,
        UM: umMap[item.UM] || item.UM,
      }));

      props.setPdn1Data(updatedPdn1Data);
    } catch (error) {
      console.error(error);
    }
    props.setShowOPDNList(false);
  ...
  useEffect(() => {
    const getOPDN = async () => {
      setIsLoading(true);
      try {
        const res = await axiosInstance.get("OPDN");
        setData(res.data);
        ...
        const respdf = await axiosInstance.get("OPDN/PDF");
  ...
  const columns = useMemo(
    () => [
      ...
      { accessorKey: "DocTotal", header: "DocTotal", size: 130 },
  ...
  const table = useMaterialReactTable({
    ...
    renderTopToolbarCustomActions: ({ table }) => (
      ...
      >
        <XLSXDownload
          fileName="List Réception De Marchandises "
