...
// ARGoodsReceiptTable component
const ARGoodsReceiptTable = React.memo((props) => {
  ...
  // Update line numbers for all entries in Pdn1Data
  useEffect(() => {
    props.setPdn1Data((prevIge12) =>
  ...
  const POR1D = () => {
    if (props.allFieldsFilled()) {
      ...
      props.setPdn1Data([
        ...
        {
          DocEntry: "",
          LineNum: props.Pdn1Data.length + 1, // Set the line number for the new entry
          ItemCode: "",
          ItemName: "",
          Quantity: 1,
          WhsCode: "",
  ...
  const handleDelete = (index) => {
    ...
    const updatedDLN1 = [...props.Pdn1Data]; // Clone the data
    updatedDLN1.splice(index, 1); // Remove the selected row
    props.setPdn1Data(updatedDLN1); // Update the state
  ...
  const handleEdit = (index, fieldName) => {
    ...
    if (props.Pdn1Data[index].isEditing || props.state.Mode !== "Créer") {
      return;
    }

    if (props.allFieldsFilled()) {
      const updatedRows = props.Pdn1Data.map((row, rowIndex) => ({
        ...row,
        isEditing: rowIndex === index,
      }));
      props.setPdn1Data(updatedRows);
                          ...
                          <FormControlLabel
                            control={
                              <Checkbox
                                ...
                                disabled={
                                  props.selectedField.name === "ItemCode" &&
                                  props.Pdn1Data[props.selectedField.index] && // UPDATE
                                  props.Pdn1Data[props.selectedField.index]
                                    .ItemCode &&
...
export default ARGoodsReceiptTable;
