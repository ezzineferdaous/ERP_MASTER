...
const AppForm = ({ setSousFamille, setshowSousFamille }) => {
  const [data, setData] = useState([{}]);
  const [isLoading, setIsLoading] = useState(false);
  const [isRefetching, setIsRefetching] = useState(false);
  const [rowCount, setRowCount] = useState(0);
  const exportColumns = ['id', 'CodeSFamille', 'NomSFamille', 'CodeFamille'];
  ...
  const handleClick = (event, row) => {
    setSousFamille(row.original);
    setshowSousFamille(false);
  };

  useEffect(() => {
    const GetSousFamille = async () => {
      setIsLoading(true);
      try {
        if (!data.length) { setIsLoading(true); } else { setIsRefetching(true); }
        const res = await axiosInstance.get('SousFamille/');
        setData(res.data);
  ...
  const columns = useMemo(
    () => [
      { accessorKey: 'CodeSFamille', header: 'Code Sous Famille', size: 150 },
      { accessorKey: 'NomSFamille', header: 'Nom Sous Famille', size: 150 },
      { accessorKey: 'CodeFamille', header: 'Famille', size: 150 },
      {
  ...
  const table = useMaterialReactTable({ columns, data, rowCount,
    renderTopToolbarCustomActions: ({ table }) => (
      <Box sx={{ display: 'flex', gap: '16px', padding: '8px', flexWrap: 'wrap' }}>
        ...
        <PDFDownload fileName="List Sous Famille" tableHeaders={exportColumns} disabled={table.getPrePaginationRowModel().rows.length === 0} data={table.getPrePaginationRowModel().rows} Text={'PDF'} />
      </Box>
    ),
