import { Stack, Snackbar, Alert } from '@mui/material';
import { Box, styled } from '@mui/system';
import { Breadcrumb, SimpleCard } from 'app/components';
import ItemGroupForm from './ItemGroupForm';
import { Menubar } from 'primereact/menubar';
import axios from 'axios';
import { useState } from 'react';
import { useNavigate } from 'react-router-dom';
import ItemGroupToolBar from './ItemGroupToolBar';
import AppItemGroupList from './AppItemGroupList';

const Container = styled('div')(({ theme }) => ({
  margin: '30px',
  [theme.breakpoints.down('sm')]: { margin: '16px' },
  '& .breadcrumb': {
    marginBottom: '30px',
    [theme.breakpoints.down('sm')]: { marginBottom: '16px' },
  },
}));

const AppForm = () => {
  const [ItemGroupe, setItemGroupe] = useState({ id: '', CodeGroupe: '', NomGroupe: '', CmptCharges: '', CmptProduit: '', });
  const [showItemGroupe, setshowItemGroupe] = useState(false);
  const [state, setState] = useState({ Mode: 'Créer', message: 'Error', open: false, vertical: 'top', horizontal: 'center', severity: 'error', CodeReadOnly: false });
  const { vertical, horizontal, open, message, severity } = state;
  const baseUrl = process.env.REACT_APP_API_BASE_URL;
  const axiosInstance = axios.create({ timeout: 5000, baseURL: baseUrl, withCredentials: true });
  const navigate = useNavigate();

  const handleClose = () => {
    setState({ ...state, open: false });
  };

  const handleChange = (event) => {
    const value = event.target.value;
    setItemGroupe({ ...ItemGroupe, [event.target.name]: value });
    console.log(ItemGroupe);
    if (ItemGroupe.id === '') {
      setState({ ...state, Mode: 'Créer', CodeReadOnly: false });
    } else {
      setState({ ...state, Mode: 'Mettre à jour', CodeReadOnly: true });
    }
  };

  const handelError = (error) => {
    if (error.response) {
      setState({ Mode: 'Créer', vertical: 'top', horizontal: 'center', open: true, message: error.response.data.message, severity: 'error', CodeReadOnly: false});
    } else if (error.request) {
      setState({ Mode: 'Créer', vertical: 'top', horizontal: 'center', open: true, message: 'Erreur de récupération des données', severity: 'error', CodeReadOnly: false });
    } else {
      setState({ Mode: 'Créer', vertical: 'top', horizontal: 'center', open: true, message: error, severity: 'error', CodeReadOnly: false });
    }
  };

  const handleSubmit = async (event) => {
    event.preventDefault();
    if (state.Mode === 'Créer') {
      try {

        axiosInstance.post('ItemGroup', ItemGroupe);
        setState({ Mode: 'Créer', vertical: 'top', horizontal: 'center', open: true, message: 'Opération correctement achevée', severity: 'success', CodeReadOnly: false });
        setItemGroupe({ id: '', CodeGroupe: '', NomGroupe: '', CmptCharges: '', GroupCode: '' });        
      } catch (error) {
        handelError(""+error);
      }
    } else if (state.Mode === 'Mettre à jour') {
      console.log(ItemGroupe);
      try {
        await axiosInstance.put('ItemGroup/' + ItemGroupe.id + '', ItemGroupe);
        setState({ Mode: 'Créer', vertical: 'top', horizontal: 'center', open: true, message: 'Opération correctement achevée', severity: 'success', CodeReadOnly: false });
        setItemGroupe({ id: '', CodeGroupe: '', NomGroupe: '', CmptCharges: '', GroupCode: '' });
      } catch (error) {
        handelError(""+error);
      }
    } else {
      navigate('/');
    }
  };

  const start = (
    <Breadcrumb
      routeSegments={[
        { name: 'Gestion des stocks'},
        { name: 'Groupe Article' },
      ]}
    />
  );
  const end = (
    <ItemGroupToolBar ItemGroupe={ItemGroupe} setItemGroupe={setItemGroupe} state={state} setState={setState} handelError={handelError} setshowItemGroupe={setshowItemGroupe} />
  );

  return (
    <Container>
      <div className="card"> 
        <Menubar lg={6} md={6} sm={12} xs={12} start={start} end={end} style={{ border: '1px solid #dee2e600' }} />
      </div>
      <Box sx={{ width: 500 }}>
        <Snackbar open={open} autoHideDuration={6000} onClose={handleClose} anchorOrigin={{ vertical, horizontal }} key={vertical + horizontal} >
          <Alert onClose={handleClose} severity={severity} sx={{ width: '100%' }} variant="filled"> {message} </Alert>
        </Snackbar>
      </Box>
      <Stack spacing={3}>
        {showItemGroupe ? (<AppItemGroupList setItemGroupe={setItemGroupe} setshowItemGroupe={setshowItemGroupe} />) : (
          <SimpleCard>
            <ItemGroupForm Mode={state.Mode} CodeReadOnly={state.CodeReadOnly} ItemGroupe={ItemGroupe} setItemGroupe={setItemGroupe} setState={setState} state={state} handleChange={handleChange} handleSubmit={handleSubmit} />
          </SimpleCard>
        )}
      </Stack>
    </Container>
  );
};

export default AppForm;
