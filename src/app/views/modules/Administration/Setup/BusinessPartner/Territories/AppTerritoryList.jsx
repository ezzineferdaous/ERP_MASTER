...
const AppForm  = ({setTerritory, setshowListTerritories}) => {
  const [data, setData] = useState([{}]);
  const [isLoading, setIsLoading] = useState(false);
  const [isRefetching, setIsRefetching] = useState(false);
  const [rowCount, setRowCount] = useState(0);
  const exportColumns = ["id", "descript"];
    ...
    const GetTerritory = async () => {
      setIsLoading(true);
        try {
          ...
          const  res  = await axiosInstance.get("Territory/");
          setData(res.data);
  ...
  const columns = useMemo(
    () => [ { accessorKey: 'descript',header: 'Description',},
    {
      ...
      header: 'Afficher',
      size: '10',Cell: ({ renderedCellValue, row }) => (
        <IconBox>
  ...
  const table = useMaterialReactTable({
    ...
    renderTopToolbarCustomActions: ({ table }) => (
      <Box sx={{ display: 'flex', gap: '16px', padding: '8px', flexWrap: 'wrap', }} >
        ...
        <PDFDownload fileName = "List Territories" tableHeaders = {exportColumns} disabled={table.getPrePaginationRowModel().rows.length === 0} data = {table.getPrePaginationRowModel().rows} Text = {'PDF'} />
      </Box>),
