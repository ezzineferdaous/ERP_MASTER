...
import AppTaxList from "./AppTaxList";
...
const AppForm = () => {
  ...
  const [Tax, setTax] = useState({ id: '', Code: '', Name: '', Rate: '', Active: 'Y' });
  const [showListTax , setshowListTax] = useState(false);
  ...
  const handleChange = (event) => {
    ...
    setTax({ ...Tax, [event.target.name]: value });
    if(Tax.id === "") {
  ...
  const handleSubmit = async (event) => {
      ...
      if(state.Mode === "Créer") {      
          ...
          try {
            await axiosInstance.post("Tax", Tax);
            ...
            setTax({id: "", Code: "", Name : "", Rate: "", Active: 'Y' });
          } catch (error) { handelError(""+error); }

      } else if(state.Mode === "Mettre à jour")  {

        try {
          await axiosInstance.put("Tax/"+Tax.id+"", Tax);
          setState({  Mode: 'Créer', vertical: 'top', horizontal: 'center', open: true, message: "Opération correctement achevée", severity: "success", CodeReadOnly: false });
          setTax({id: "", Code: "", Name : "", Rate: "", Active: 'Y' });
        } catch (error) { handelError(""+error); }
  ...
  const start = <Breadcrumb routeSegments={[{ name: "Administration", path: "/Tax" }, { name: "TVA" }]} />;
  const end = <TaxToolBar Tax = {Tax} setTax ={setTax} state ={state} setState ={setState} handelError={handelError} setshowListTax = {setshowListTax} /> 
  ...
  return (
    <Container>      
      ...
      <Stack spacing={3}>
        { showListTax ? (
          <AppTaxList setTax ={setTax} setshowListTax = {setshowListTax} />
         ) : (
            <SimpleCard>
              <TaxForm  Mode= {state.Mode} Tax={Tax} CodeReadOnly={state.CodeReadOnly} handleChange={handleChange} handleSubmit={handleSubmit} />
            </SimpleCard>
         )}
      </Stack>
    </Container>
