...
// const IGN1voiceTable = React.memo((props) => {
//     // add  thes varibel  
...
//     let updatedIGN1 = [...props.IGN1];
//     // Calculate LineTotal when Quantity, Price, or Discount changes
//       let Quantity = parseFloat(updatedIGN1[index].Quantity) || 0;
//       let prix = parseFloat(updatedIGN1[index].Price) || 0;
//       let discount = parseFloat(updatedIGN1[index].Discount) || 0;
//       let lineTotal = (Quantity * prix) - discount;
//       // updatedIGN1[index].LineTotal = lineTotal.toFixed(2);
...
//     // Set the updated IGN1 array in the state
//     props.setIGN1(updatedIGN1);
//   };
  
//      // send the value in the input
//    const handleInputChange = (index, event) => {
//     const { name, value } = event.target;
//     let updatedIGN1 = [...props.IGN1];
//     updatedIGN1[index] = { ...updatedIGN1[index], [name]: value };
//     props.setIGN1(updatedIGN1);
//   };
...
//       props.setEditIndex(props.IGN1.length);
//       props.fetchNextDocEntry();
//       props.setIGN1([
//         ...props.IGN1,
//         {
//           DocEntry: "",
//           LineNum: props.IGN1.length + 1,
//           ItemCode: "",
//           ItemName: "",
//           Quantity: 1,
//           WhsCode: "",
...
//   if (props.IGN1.length <= 1) {
//     props.setState({
//       ...props.state,
//       open: true,
//       message: "Il doit rester au moins une ligne",
//       severity: "warning"
//     });
//     return;
//   }

//   const updatedIGN1 = [...props.IGN1];
//   updatedIGN1.splice(index, 1); // Remove the selected row
//   props.setIGN1(updatedIGN1); // Update the state with the new list
//   console.log("Data after delete:", updatedIGN1);
// };

// // updates the LineNum

// useEffect(() => {
//   props.setIGN1((prevIge12) =>
...
//   if (props.IGN1[index].isEditing || props.state.Mode !== "Créer") {
//      return;
//   }

//   if (props.allFieldsFilled()) {
//      const updatedRows = props.IGN1.map((row, rowIndex) => ({
//         ...row,
//         isEditing: rowIndex === index,
//      }));
//      props.setIGN1(updatedRows);
...
//                 props.state.Mode !== "OK" && <TableCellK>Action</TableCellK>}         
//                 </TableRow>
//        </TableHead>
//      <TableBody>
//        {props.IGN1.map((apin, index) => (
//     // componant table  
...
//                    <FontAwesomeIcon
//                      icon={faBars}
//                      onClick={() => props.handleOpenModal('WhsCode', 'WhsCode', index)}
//                      style={{ position: 'absolute', right: '10px', cursor: 'pointer' }}
//                    />
//                  </div>
//                </StyledTableCell>
...
//                     defaultValue={apin.Discount || 0} // Ensure a default value
//                     onFocus={(e) => handleFocus(e.target.value)}
//                     onBlur={(e) => handleBlur(index, e)}
//                     onChange={(e) => handleInputChange(index, e)} // Update state on change
//                     disabled={!apin.ItemCode} // Prevent input if ItemCode is missing
//                     InputProps={{
//                       style: { borderRadius: "0px", appearance: "textfield" },
//                     }}
//                   />
//                 ) : (
//                   <span>{apin.Discount || 0}</span> // Display the value if not editing
//                 )}
//               </StyledTableCell>
//                <StyledTableCell>
//                <TextField
...
//               <StyledTableCell name="WhsCode" data-field="WhsCode" style={{ textAlign: "center" }}>{apin.WhsCode}</StyledTableCell>
//               <StyledTableCell data-field="Price" style={{ textAlign: "center" }}>{apin.Price}</StyledTableCell>
//               <StyledTableCell data-field="Discount" style={{ textAlign: "center" }}>{apin.Discount}</StyledTableCell>
...
//                                   disabled={
//                                     props.selectedField.name === "ItemCode" &&
//                                     props.IGN1[props.selectedField.index] &&
//                                     props.IGN1[props.selectedField.index].ItemCode &&
...
const handleCalculations = (index) => {
  let updatedige1 = [...props.IGN1];

  let Quantity = parseFloat(updatedige1[index].Quantity) || 0;
  let prix = parseFloat(updatedige1[index].Price) || 0;
  let discount = parseFloat(updatedige1[index].Discount) || 0;
  let discountedAmount = ((prix * discount) / 100).toFixed(2);

  prix -= parseFloat(discountedAmount);
  ...
  props.setIGN1(updatedige1.map((item, i) => ({
    ...item,
    LineNum: i + 1,
  })));
...
const handleInputChange = (index, event) => {
  const { name, value } = event.target;
  const updatedige1 = props.IGN1.map((ige, i) =>
    i === index ? { ...ige, [name]: value } : ige
  );
  props.setIGN1(updatedige1);
  ...
  const addige = () => {
    if (props.allFieldsFilled()) {
      const updatedRows = props.IGN1.map(row => ({ ...row, isEditing: false }));

      props.fetchNextDocEntry();
      props.setIGN1([...updatedRows, { DocEntry: '', LineNum:props.IGN1.length + 1, ItemCode: '', ItemName: '', Quantity: '1', WhsCode: '', Price: '0.00', Discount: '0',  LineTotal: '0.00' ,UM: '',isEditing: true, }]);
    } else {
      props.setState({ ...props.state, open: true, message: "Please choose Code Article before adding a new row.", severity: "error" });
    }
  };
  ...
  //   props.IGN1.splice(index , 1);
  //   props.setIGN1([...props.IGN1]);
  //   console.log("Data to save:", props.IGN1);
  // };
  const handleDelete = (index) => {
    if (props.IGN1.length <= 1) {
    ...
    const updatedPOR1 = [...props.IGN1];
    updatedPOR1.splice(index, 1);
    props.setIGN1(updatedPOR1);
  ...
  //     const updatedRows = props.IGN1.map((row, rowIndex) => ({
  //       ...row,
  //       isEditing: rowIndex === index, // Set isEditing to true only for the specified index
  //     }));
  //     console.log("edite", updatedRows);
      
  //     props.setIGN1(updatedRows); // Update the rows with the new editing state
  //   } else {
  //     alert("Please fill all fields before adding a new row!");
  //   }
  // };
            ...
            <TableRow key={index}              
              ...
              <StyledTableCell data-field="Action">
                ...
                <>
                  ...
                  <StyledTableCell name="WhsCode" data-field="WhsCode" >{ige.WhsCode}</StyledTableCell>
                  <StyledTableCell name="Price" data-field="Price" >{ige.Price}</StyledTableCell>
                  <StyledTableCell name="Discount" data-field="Discount" >{ige.Discount}</StyledTableCell>
