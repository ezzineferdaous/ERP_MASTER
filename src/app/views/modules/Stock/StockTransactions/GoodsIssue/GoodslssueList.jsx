...
const GoodsIssueList = ({setige1, setState ,state,oige, setOige, setshowListOige }) => {
  ...
  const handleClick = async (event, row) => {
    try {
      ...
      setOige(row.original);
  
      // Fetch IGE1 data based on the selected DocEntry
      const ige1Response = await axiosInstance.get(`IGE1/data/Min/${row.original.DocEntry}`);
  ...
  useEffect(() => { 
    const GetGoodsIssue = async () => {
      ...
      try {
        ...
        const res = await axiosInstance.get("OIGE/");
        setData(res.data);
