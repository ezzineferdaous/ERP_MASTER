...
const GoodsIssueFormIge1 = (props) => {
...
const handleCalculations = (index) => {
  ...
  props.setige1(updatedige1.map((item, i) => ({
    ...item,
    LineNum: i + 1,
  })));
...
const handleInputChange = (index, event) => {
  ...
  const updatedige1 = props.ige1.map((ige, i) =>
    i === index ? { ...ige, [name]: value } : ige
  );
  props.setige1(updatedige1);
  ...
  const addige = () => {
    if (props.allFieldsFilled()) {
      const updatedRows = props.ige1.map(row => ({ ...row, isEditing: false }));

      props.fetchNextDocEntry();
      props.setige1([...updatedRows, { DocEntry: '', LineNum:props.ige1.length + 1, ItemCode: '', ItemName: '', Quantity: '1', WhsCode: '', Price: '0.00', Discount: '0',  LineTotal: '0.00' ,UM: '',isEditing: true, }]);
    } else {
  ...
  //   props.ige1.splice(index , 1);
  //   props.setige1([...props.ige1]);
  //   console.log("Data to save:", props.ige1);
  // };
  const handleDelete = (index) => {
    if (props.ige1.length <= 1) {
      props.setState({ ...props.state, open: true, message: "Il doit rester au moins une ligne ", severity: "warning" });
      return;  
    }
    const updatedPOR1 = [...props.ige1];
    updatedPOR1.splice(index, 1);
    props.setige1(updatedPOR1);
  ...
  //   if (props.ige1[index].isEditing || props.state.Mode !== "Créer") {
  //     return; // Exit the function if the conditions are not met
  //   }
  //   if (props.allFieldsFilled()) {
  //     const updatedRows = props.ige1.map((row, rowIndex) => ({
  //       ...row,
  //       isEditing: rowIndex === index, // Set isEditing to true only for the specified index
  //     }));
  //     console.log("edite", updatedRows);
      
  //     props.setige1(updatedRows); // Update the rows with the new editing state
  //   } else {
                    ...
                    <TableRow key={index} onClick={(e) => { if (e.target.type !== 'checkbox') { props.handleSelectItem(item);}}} style={{ cursor: 'pointer' }}>
                      ...
                      {props.selectedField.name === "ItemCode" ? (
                        ...
                        control={
                          ...
                          props.ige1[props.selectedField.index] && 
                          props.ige1[props.selectedField.index].ItemCode && 
...
export default GoodsIssueFormIge1;
