...
const AppForm  = ({setPartner, setshowListPartners}) => {
  const [data, setData] = useState([{}]);
  const [isLoading, setIsLoading] = useState(false);
  const [isRefetching, setIsRefetching] = useState(false);
  const [rowCount, setRowCount] = useState(0);
  const exportColumns = ["CardCode", "CardName", "CardType","GroupCode"];
  ...
  const columns = useMemo(
    () => [ 
      { accessorKey: 'CardCode',header: 'Code de Partenaire',size: 150, }, 
      { accessorKey: 'CardName',header: 'Nom de Partenaire',size: 150, },
      { accessorKey: 'CardType',header: 'Type de Partenaire',size: 150, accessorFn: (row) => row.CardType === 'C' ? 'Client' :  row.CardType === 'S' ? 'Fournisseur' : ''  },
      { accessorKey: 'GroupCode',header: 'Groupe Partenaire',size: 150 },
      {
