...
exports.create = (req, res)  => {
    ...
    //Save to database
    POR1.create(pOR1, { fields: ['DocEntry', 'LineNum', 'ItemCode', 'ItemName', 'Quantity', 'WhsCode','VAT','PriceHT', 'Price', 'Discount','RemiseTotal','LineTotal','LineHT', 'UM'] }).then(
        data => {
            res.send(data);
        }).catch(err => {
            res.status(500).send({
                message: err.message || "Erreur lors de la création de POR1"
            });
        });
};

//Retrive all POR1s from the database by Name
exports.findAll = (req, res)  => {
    
    const DocEntry = req.query.DocEntry;
    var condition = DocEntry ? { DocEntry : { [Op.like]: `%${DocEntry}%` } } : null;

    POR1.findAll({ where : condition})
    .then(data => {
            res.send(data);
    })
    .catch(err => {
        res.status(500).send ({
            message: err.message || "Erreur lors de la selection des POR1"
        });
    });
};

//Find a single POR1 with an id = Code
exports.findOne = (req, res)  => {
    ...
    .catch(err => {
        res.status(500).send({
            message: err.message || "Erreur lors de la selection de POR1 : " + id
        });
    });
};
...
//Update a POR1 by the id passed by the request


exports.update = (req, res)  => {

    const docEntry = req.params.docEntry;
    POR1.update(req.body, {
