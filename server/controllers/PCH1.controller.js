...
exports.create = (req, res)  => {
    ...
    //Save to database
    PCH1.create(pCH1, { fields:['DocEntry', 'LineNum', 'ItemCode', 'ItemName', 'Quantity', 'WhsCode','VAT','PriceHT', 'Price', 'Discount','RemiseTotal','LineTotal','LineHT', 'UM'] }).then(
        data => {
            res.send(data);
        }).catch(err => {
            res.status(500).send({
                message: err.message || "Erreur lors de la création de PCH1"
            });
        });
};

//Retrive all PCH1s from the database by Name
exports.findAll = (req, res)  => {
    
    const DocEntry = req.query.DocEntry;
    var condition = DocEntry ? { DocEntry : { [Op.like]: `%${DocEntry}%` } } : null;

    PCH1.findAll({ where : condition})
    .then(data => {
            res.send(data);
    })
    .catch(err => {
        res.status(500).send ({
            message: err.message || "Erreur lors de la selection des PCH1"
        });
    });
};

//Find a single PCH1 with an id = Code
exports.findOne = (req, res)  => {
    ...
    .catch(err => {
        res.status(500).send({
            message: err.message || "Erreur lors de la selection de PCH1 : " + id
        });
    });
};
...
//Update a PCH1 by the id passed by the request


exports.update = (req, res)  => {

    const docEntry = req.params.docEntry;
    PCH1.update(req.body, {
