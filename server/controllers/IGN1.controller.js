...
exports.create = (req, res)  => {
    ...
    //Save to database
    IGN1.create(newiGN1, { fields: ['DocEntry', 'LineNum', 'ItemCode', 'ItemName', 'Quantity', 'WhsCode', 'Price', 'Discount', 'VAT', 'LineTotal', 'UM'] }).then(
        data => {
            res.send(data);
        }).catch(err => {
            res.status(500).send({
...
//Find a single IGN1 with an id = Code
...
//Update a IGN1 by the id passed by the request
exports.update = (req, res)  => {

    const id = req.params.id;
    IGN1.update(req.body, {
        where : {id: id }
    }).then(num => {
        if(num == 1) {
            res.send({
...
exports.getMin = (req, res)  => {
  ...
  IGN1.findAll({ where: { DocEntry: docEntry }})
  .then(ign1s => {
      res.send(ign1s);        
  }).catch(err => {
      res.status(500).send({
