...
exports.create = (req, res)  => {
    ...
    //Save to database
    ORDR.create(oRDR, { fields: ['DocEntry', 'DocNum', 'DocDate', 'DueDate', 'CardCode', 'CardName', 'TotalHT', 'DiscPrcnt','RemiseTotal','VatSum', 'DocTotal', 'Comment', 'Canceled', 'DocStatus', 'UserSign'] }).then(
        data => {
            res.send(data);
        }).catch(err => {
            res.status(500).send({
                message: err.message || "Erreur lors de la création de ORDR"
            });
        });
};

//Retrive all ORDRs from the database by Name
exports.findAll = (req, res)  => {
    
    const DocEntry = req.query.DocEntry;
    var condition = DocEntry ? { DocEntry : { [Op.like]: `%${DocEntry}%` } } : null;

    ORDR.findAll({ where : condition})
    .then(data => {
            res.send(data);
    })
    .catch(err => {
        res.status(500).send ({
            message: err.message || "Erreur lors de la selection des ORDR"
        });
    });
};
...
//Find a single ORDR with an id = Code
exports.findOne = (req, res)  => {
    ...
    .catch(err => {
        res.status(500).send({
            message: err.message || "Erreur lors de la selection de ORDR : " + id
        });
    });
};
//
...
//Update a ORDR by the id passed by the request
exports.update = (req, res)  => {

    const id = req.params.id;
    ORDR.update(req.body, {
        where : {id: id }
    }).then(num => {
        if(num == 1) {
            res.send({
...
exports.getPrevious = (req, res)  => {
    const id = req.params.id;
    ORDR.findOne({  order: [['id', 'DESC']], where: { id: { [Op.lt]: id }}})
    .then(id => {
        res.send({ id: id });        
    }).catch(err => {
        res.status(500).send({
...
exports.getNext = (req, res)  => {
    const id = req.params.id;
    ORDR.findOne({ where: { id: { [Op.gt]: id }}})
    .then(id => {
        res.send({ id: id });        
    }).catch(err => {
        res.status(500).send({
