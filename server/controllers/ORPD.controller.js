...
exports.create = (req, res)  => {
    ...
    //Save to database
    ORPD.create(NewORPD, { fields: ['DocEntry', 'DocNum', 'DocDate', 'DueDate', 'CardCode', 'CardName', 'TotalHT','DiscPrcnt', 'RemiseTotal', 'VatSum', 'DocTotal', 'Comment', 'Canceled', 'DocStatus', 'UserSign'] }).then(
        data => {
            res.send(data);
        }).catch(err => {
            res.status(500).send({
                message: err.message || "Erreur lors de la création de ORPD"
            });
        });
};

//Retrive all ORPD from the database by Name
exports.findAll = (req, res)  => {
   
    const DocEntry = req.query.DocEntry;
    var condition = DocEntry ? { DocEntry : { [Op.like]: `%${DocEntry}%` } } : null;

    ORPD.findAll({ where : condition})
    .then(data => {
            res.send(data);
    })
    .catch(err => {
        res.status(500).send ({
            message: err.message || "Erreur lors de la selection des ORPD"
        });
    });
};
...
//Find a single ORPD with an id = Code
exports.findOne = (req, res)  => {
    ...
    .catch(err => {
        res.status(500).send({
            message: err.message || "Erreur lors de la selection de ORPD : " + id
        });
    });
};
//
...
//Update a ORPD by the id passed by the request
exports.update = (req, res)  => {

    const id = req.params.id;
    ORPD.update(req.body, {
        where : {id: id }
    }).then(num => {
        if(num == 1) {
            res.send({
...
exports.getPrevious = (req, res)  => {
    const id = req.params.id;
    ORPD.findOne({  order: [['id', 'DESC']], where: { id: { [Op.lt]: id }}})
    .then(id => {
        res.send({ id: id });        
    }).catch(err => {
        res.status(500).send({
...
exports.getNext = (req, res)  => {
    const id = req.params.id;
    ORPD.findOne({ where: { id: { [Op.gt]: id }}})
    .then(id => {
        res.send({ id: id });        
    }).catch(err => {
        res.status(500).send({
