...
exports.create = (req, res)  => {
    ...
    //Save to database
    PDN1.create(pDN1, { fields:['DocEntry', 'LineNum', 'ItemCode', 'ItemName', 'Quantity', 'WhsCode','VAT','PriceHT', 'Price', 'Discount','RemiseTotal','LineTotal','LineHT', 'UM'] }).then(
        data => {
            res.send(data);
        }).catch(err => {
            res.status(500).send({
                message: err.message || "Erreur lors de la création de PDN1"
            });
        });
};


//Retrive all PDN1 from the database by Name
exports.findAll = (req, res)  => {
    
    const DocEntry = req.query.DocEntry;
    var condition = DocEntry ? { DocEntry : { [Op.like]: `%${DocEntry}%` } } : null;

    PDN1.findAll({ where : condition})
    .then(data => {
            res.send(data);
    })
    .catch(err => {
        res.status(500).send ({
            message: err.message || "Erreur lors de la selection des PDN1"
        });
    });
};

//Find a single PDN1 with an id = Code
exports.findOne = (req, res)  => {
    ...
    .catch(err => {
        res.status(500).send({
            message: err.message || "Erreur lors de la selection de PDN1 : " + id
        });
    });
};
...
//Update a PDN1 by the id passed by the request


exports.update = (req, res)  => {

    const docEntry = req.params.docEntry;
    PDN1.update(req.body, {
