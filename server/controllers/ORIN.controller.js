...
exports.create = (req, res)  => {
    ...
    //Save to database
    ORIN.create(oRIN, { fields: ['DocEntry', 'DocNum', 'DocDate', 'DueDate', 'CardCode', 'CardName', 'totalHT','DiscPrcnt' ,'RemiseTotal', 'VatSum', 'DocTotal', 'Comment', 'Canceled', 'DocStatus', 'UserSign'] }).then(
        data => {
            res.send(data);
        }).catch(err => {
            res.status(500).send({
                message: err.message || "Erreur lors de la création de ORIN"
            });
        });
};

//Retrive all ORIN from the database by Name
exports.findAll = (req, res)  => {
    
    const DocEntry = req.query.DocEntry;
    var condition = DocEntry ? { DocEntry : { [Op.like]: `%${DocEntry}%` } } : null;

    ORIN.findAll({ where : condition})
    .then(data => {
            res.send(data);
    })
    .catch(err => {
        res.status(500).send ({
            message: err.message || "Erreur lors de la selection des ORIN"
        });
    });
};
...
//Find a single ORIN with an id = Code
exports.findOne = (req, res)  => {
    ...
    .catch(err => {
        res.status(500).send({
            message: err.message || "Erreur lors de la selection de ORIN : " + id
        });
    });
};


//Update a ORIN by the id passed by the request
exports.update = (req, res)  => {

    const id = req.params.id;
    ORIN.update(req.body, {
        where : {id: id }
    }).then(num => {
        if(num == 1) {
            res.send({
...
exports.getPrevious = (req, res)  => {
    const id = req.params.id;
    ORIN.findOne({  order: [['id', 'DESC']], where: { id: { [Op.lt]: id }}})
    .then(id => {
        res.send({ id: id });        
    }).catch(err => {
        res.status(500).send({
...
exports.getNext = (req, res)  => {
    const id = req.params.id;
    ORIN.findOne({ where: { id: { [Op.gt]: id }}})
    .then(id => {
        res.send({ id: id });        
    }).catch(err => {
        res.status(500).send({
